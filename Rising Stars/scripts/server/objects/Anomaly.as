...
Anomaly@ createPlanetAnomaly(Planet@ planet, uint typeID) {
	ObjectDesc desc;
	desc.type = OT_Anomaly;
	desc.name = getAnomalyType(typeID).name;
	desc.flags |= objNoDamage;
	...
	anomaly.rotation = quaterniond_fromAxisAngle(random3d(1.0), randomd(-pi,pi));
	anomaly.setup(typeID);
	planet.addAnomaly(anomaly);
	anomaly.planet = planet;
	return anomaly;
}
...
tidy class AnomalyScript {
	...
	Planet@ get_planet(Anomaly& anomaly) {
		return planet;
	}

	void set_planet(Anomaly& anomaly, Planet& value) {
		@planet = value;
	}
	...
	string get_sprite(Player& player, const Anomaly& obj) {
		if(type is null)
			return "";
		else if(get_progress(player, obj) >= 1.f && state !is null && state.spriteName.length > 0)
	...
	void load(Anomaly& obj, SaveFile& file) {
		...
		if(stateId < type.states.length)
			@state = type.states[stateId];
		@planet = cast<Planet@>(file.readObject());
		if (planet !is null)
			planet.addAnomaly(obj);
	}
