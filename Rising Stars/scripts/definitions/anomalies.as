...
export AnomalyType, AnomalyOption, AnomalyState, AnomalyKind, PlanetAnomalyType;
export getAnomalyType, getAnomalyTypeCount, getAnomalyTypes;
export getDistributedAnomalyType;
export Targets;

enum AnomalyKind {
	AK_SpatialAnomaly,
	AK_TerrestrialAnomaly,
};
...
tidy class PlanetAnomalyType : AnomalyType {
	PlanetAnomalyType() {
		modelName = ""; matName = "";
	}
}

class AnomalyTypeList {
	AnomalyType@[] types;
	double totalFrequency = 0;

	void add(AnomalyType@ type) {
...
void loadAnomaly(const string& filename) {
	...
	while(file++) {
		...
		if(file.fullLine) {
			...
			state.spriteName = anomaly.spriteName;
...
void preInit() {
	//Initialize lists
	@spaceAnomalyTypes = AnomalyTypeList();
	@terrestrialAnomalyTypes = AnomalyTypeList();
	//Load anomaly types
...
dictionary idents;
AnomalyTypeList@ spaceAnomalyTypes;
AnomalyTypeList@ terrestrialAnomalyTypes;

AnomalyTypeList@ getAnomalyTypes(AnomalyKind kind) {
	switch (kind) {
		case AK_SpatialAnomaly:
			return spaceAnomalyTypes;
		case AK_TerrestrialAnomaly:
			return terrestrialAnomalyTypes;
		default: error("Undefined anomaly kind");
	}
	return spaceAnomalyTypes;
}
...
uint getAnomalyTypeCount() {
	return getAnomalyTypeCount(spaceAnomalyTypes);
}

uint getAnomalyTypeCount(AnomalyTypeList& list) {
	return list.types.length;
}

const AnomalyType@ getDistributedAnomalyType() {
	return getDistributedAnomalyType(spaceAnomalyTypes);
}

const AnomalyType@ getDistributedAnomalyType(AnomalyTypeList& list) {
...
void addAnomalyType(AnomalyType@ type) {
	...
	PlanetAnomalyType@ planetType = cast<PlanetAnomalyType@>(type);
	if (planetType is null)
		spaceAnomalyTypes.add(type);
	else
		terrestrialAnomalyTypes.add(type);
	print("add " + type.id + " name = " + type.name + " length = " + anomalyTypes.length);
}
